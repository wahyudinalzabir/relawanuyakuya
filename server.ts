...
async function startServer() {
  ...
  const requireAuthUser = (req: Request, res: Response): User | null => {
    ...
    if (!user) {
      // Fallback for seamless demo or initial load if needed, or enforce login
      const defaultUser = db.getUserById('USR-KORCAM-01') || db.getUsers()[0];
  ...
  app.post('/api/export/sheets', (req: Request, res: Response) => {
    ...
    try {
      ...
      // 24 Columns specified in requirement #11:
      const headers = [
        'ID Relawan',
        'NIK',
        'Nama',
      ...
      const rows = items.map((r) => [
        r.id_relawan || '',
        `'${r.nik}`, // prefix with quote to prevent excel stripping leading zeroes
        r.nama || '',
        r.tempat_lahir || '',
        r.tanggal_lahir || '',
        r.jenis_kelamin || '',
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Sistem Pendataan Relawan berjalan di http://0.0.0.0:${PORT}`);
  });
}
